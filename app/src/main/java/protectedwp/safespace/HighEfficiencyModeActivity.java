...
public class HighEfficiencyModeActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toggle.setText("Enable High-Efficiency Mode? In this mode, the app will use alarms and receivers to continuously restart itself and maintain a high process priority, to make it harder for aggressive firmware to kill it and ensure its restart after death. The app will also launch 10 short WakeLocks, which will be constantly refreshed to prevent the phone from entering Doze Mode. This is necessary for the app to respond quickly to threats, as USB event broadcasts may arrive slowly in Doze Mode. Furthermore, when the main service restarts, not just in the screen-off receiver — the app will check if the screen is off or locked and the profile is not encrypted. Put simply, it will check if the app missed the screen turning off moment which occurred when the receiver was inactive and app didn't have trigger to evict keys, for example because it was killed at that moment and only then restarted. in this case, it will evict them using the same mechanism DevicePolicyManager.lockNow(FLAG_EVICT_CREDENTIAL_ENCRYPTION_KEY).\n\n"+
                ...
                "The high-efficiency mode status saves in the app’s memory, but changes to this status only take effect after the app’s process has been restarted. To restart the app’s process, it is recommended that you simply restart the phone.\n\n"                
